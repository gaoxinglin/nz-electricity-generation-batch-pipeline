...
SNOWFLAKE_PUBLIC_KEY_PATH_FOR_TERRAFORM="${SNOWFLAKE_PUBLIC_KEY_PATH:-}"
if [[ -z "$SNOWFLAKE_PUBLIC_KEY_PATH_FOR_TERRAFORM" && "$SNOWFLAKE_PRIVATE_KEY_PATH_FOR_TERRAFORM" == *.p8 ]]; then
  SNOWFLAKE_PUBLIC_KEY_PATH_CANDIDATE="${SNOWFLAKE_PRIVATE_KEY_PATH_FOR_TERRAFORM%.p8}.pub"
  if [[ -f "$SNOWFLAKE_PUBLIC_KEY_PATH_CANDIDATE" ]]; then
    SNOWFLAKE_PUBLIC_KEY_PATH_FOR_TERRAFORM="$SNOWFLAKE_PUBLIC_KEY_PATH_CANDIDATE"
  fi
fi

if [[ -n "$SNOWFLAKE_PUBLIC_KEY_PATH_FOR_TERRAFORM" && -z "${TF_VAR_snowflake_public_key_path:-}" ]]; then
  export TF_VAR_snowflake_public_key_path="$SNOWFLAKE_PUBLIC_KEY_PATH_FOR_TERRAFORM"
fi

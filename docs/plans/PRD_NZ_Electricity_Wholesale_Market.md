...
| **6.0-impl** | **2026-05-14** | **V3 Hydro Integration Done — 反转 v4.2 减法**（12 子任务全过；V2 Out of Scope §11 第一条 Hydro 项落地）。**重大 PRD 偏差与处理**：(1) HMD 实际路径是 `Datasets/Environment/HydrologicalModellingDataset/3_StorageAndSpill_{YYYYMMDD}/3_1_Storage/`，不在 PRD §2 隐含的 `Wholesale/...` 路径下；**定期发布**（≈ 年度），不像 generation/price 是月度文件 —— 最新发布日 `20241231`（数据覆盖至 2024-12-31，发布于 2025-10-30）。(2) 数据按湖泊分文件（10 个 storage CSV，每个 ~660KB）+ 一个 `FileIndex_Storage.csv` 索引；湖泊文件名 = `{IslandCode}_{SiteCode}_Storage_{LakeName}.csv`。(3) CSV header 含 unicode 字符（`Lake level (m)`, `Active storage (Mm³)`），通过 `load_local.py` pandas 层 `HYDRO_COLUMN_MAP` 做列名标准化（避开 DuckDB / SF 对带 `³` 列名的引用复杂度）。(4) NI 只有 **2 个**主要 storage lake（Taupo + Waikaremoana），SI 有 **8 个**（Tekapo/Pukaki/Ohau/Hawea/Wanaka/Wakatipu/Manapouri/Te Anau）—— 10 行 seed 表替代 PRD §10.1 担忧的"catchment→island 手工近似"，实际归属由 FileIndex 的 `IslandCode` 列直接给出，无歧义。(5) 真实 storage 容量参考表存在于 HMD `1_Infrastructure.../EnergyStorageCapacity(DerivedForScheme).csv`，但本次未接入；改用 `MAX(avg_total_storage_mm3) OVER (PARTITION BY island)` 派生 `storage_pct_of_max`（历史观测最大值占比，语义略弱于"占额定容量百分比"但实现成本零）。**交付**：(a) `scripts/download_hydro.py` —— 按已知 release 列表（newest first：20241231 → 20231231 → …）HEAD 探测 → 抓 FileIndex_Storage.csv → 下载 10 个湖泊 CSV 到 `data/raw/hydro/`，写 `_release.txt` 标记版本。(b) `scripts/load_local.py` 扩展 `load_hydro()` + `load_hydro_file()` —— pandas 读 CSV → `HYDRO_COLUMN_MAP` 列名重命名 → 事务式 `DELETE WHERE site_code=? + INSERT`，幂等 per-site；filename regex `([A-Z]{2}_[A-Z]{2,4})_Storage_.*\.csv$` 提取 site_code 注入。(c) `dbt/seeds/hydro_catchment_mapping.csv` — 10 行：site_code / catchment_name / island / plant_group / scheme。(d) `dbt/models/staging/stg_hydro_storage.sql` — `TRY_CAST` 跨 SF/DuckDB 兼容、过滤到 2016-01-01+ 与 active_storage_mm3 IS NOT NULL（DuckDB 实跑 **32,880 行** = 10 湖 × ~3,288 日）。(e) `dbt/models/core/dim_catchment.sql` — 10 行 surrogate-key 维度（`dbt_utils.generate_surrogate_key` on site_code）。(f) `dbt/models/core/fct_hydro.sql` — `materialized='table'`（HMD 全量重载语义），grain = `site_code × trading_date`，LEFT JOIN dim_catchment 注入 island/scheme（NI 6,576 行 + SI 26,304 行 = 32,880 行）；`date_key` 用 `EXTRACT(YEAR)*10000 + EXTRACT(MONTH)*100 + EXTRACT(DAY)` 跨方言派生。(g) `dbt/models/core/mart_hydro_price_driver.sql` — 三 CTE 链：`island_daily` 跨湖泊 SUM → `hydro_monthly` 月度聚合（AVG/MIN/MAX/observation_days）→ `hydro_with_pct` window 函数加 `storage_pct_of_max`；最后 LEFT JOIN `mart_price_daily` 的 island×month 价格聚合（**108 月 × 2 岛 = 216 行**，demo 模式下 price 全 NULL）。(h) `streamlit/pages/hydro_price_driver.py`（**第 11 个 dashboard 页**，💧 V3 分组）—— Island multi-select + 双 KPI 行（storage Mm³ + delta 占 max % / price $/MWh）+ 双 Y 轴时间序列（柱状储量 + 折线电价，via `make_subplots(secondary_y=True)`）+ 散点图含每岛 Pearson r metric + 折叠的 per-lake 明细 line chart（facet by island）。`has_price` 判断分支：无价格重叠时仅画储量 + 提示 `make local-full`。(i) `streamlit/loader.py` 加 `load_hydro_price_driver()` + `load_hydro_storage_detail()` 两 loader。(j) `streamlit/app.py` 导航加 "Hydro (V3)" 顶层分组。(k) `airflow/dags/nz_electricity_v2.py` 第 **4 条** ingest 分支 `hydro_download → hydro_upload → hydro_load`（NONE_FAILED 触发，与 NSP 并列 best-effort）；download 用 `AirflowSkipException` 实现"无 release 时跳过"；load 用 `TRUNCATE + COPY` 全量重载（与 NSP 同 pattern）；topology 改为 `[g_load, p_load, n_load, h_load] >> t_check_dbt`。(l) `Makefile` demo / local-full / local-subset 三 target 加 hydro download；新增 ingest 后的二次 `dbt run --select stg_dbt_run fct_dbt_run` 处理 observability bootstrap 链。**测试**：`schema.yml` 加 `stg_hydro_storage` / `dim_catchment` / `fct_hydro` / `mart_hydro_price_driver` 全套（not_null / unique / accepted_values + 2 个 `dbt_utils.unique_combination_of_columns`）；`tests/test_dag_integrity.py` 加 `hydro_*` 3 task 到 v2 expected set + 把 `test_v2_branch_join` 上游集合改为 4 路收束（含 `hydro_load`）。**端到端**：DuckDB 全量 `dbt run` **27/27 PASS**（V2 23 + V3 hydro 4）、专项 `dbt test --select` hydro 模型组 **21/21 PASS**；Streamlit `/_stcore/health = ok`，第 11 页可加载 mart 216 行。**实测发现 vs PRD**：(1) PRD §11 估算 Hydro 工作量 ~3 天，实际 < 1 天（V2 已搭好双模式骨架 + cross-db macros + dim_node mart pattern 可直接套用）。(2) Demo 模式（1 个月 price = 2026-04）与 HMD（2016-2024）**无时间重叠** → `mart_hydro_price_driver.avg_price_nzd_mwh` 全 NULL，散点图为空；Streamlit `has_price=False` 分支 + 显式提示 `make local-full`；mart 本身仍正确构建 216 行储量数据。(3) PRD §10.1 "catchment→island 映射手工近似"担忧在实际数据上不成立 —— 10 个湖泊 island 归属固定且权威（FileIndex.IslandCode）。**累计项目状态**：27 dbt models（V2 23 + V3 hydro 4）+ 4 cross-db macros + **11 streamlit pages**（10 V1/V2 + 1 V3 hydro）+ 2 DAGs（V2 现 16 task）+ **7 scripts**（新增 download_hydro.py）+ **3 seeds**（新增 hydro_catchment_mapping.csv）。**留给后续 V3.x**：(1) Bids and Offers 数据接入（§11 第二条）；(2) Observability Tier 2 剩余 — S3 存储 mart + 行数/价格 drift detection + dbt docs CI → GitHub Pages；(3) HMD `1_Infrastructure.../EnergyStorageCapacity(DerivedForScheme).csv` 真实容量参考 → 把 `storage_pct_of_max` 换成 `storage_pct_of_capacity` 的更精确版；(4) `2_Flows_{date}/` 流入数据（cumecs 流量）建模 hydro 入库率 leading indicator，当前只有存量快照；(5) Snowflake 端 `raw_hydro_storage` Terraform DDL + V2 DAG hydro_load 路径在 Cloud 模式实测（本次只跑通 DuckDB，SF parse + DAG bag import 已过但 COPY INTO 未实测） |
| **6.1-impl** | **2026-05-14** | **Observability bootstrap fix —— 消除 92.6% model-success 永久噪声**（3 子任务）。**根因**：5.7-impl 引入的 `stg_dbt_run` → `fct_dbt_run` 链依赖 `raw_dbt_run` 源表，但首次 `dbt run` 时该表还不存在 → 强制 1 error（stg_dbt_run）+ 1 skipped（fct_dbt_run，因上游失败被自动跳过）写入观察性表。结果是 Pipeline Health 页"Model success 30d" KPI 永久卡在 **25/27 = 92.59%**（< 95% SLO ❌），需要 ≥ 5 次后续干净 invocation 才能稀释回绿色 —— 等价于"全新部署即报警"。**修复**：(a) `scripts/ingest_dbt_artifacts.py` 加 `--init` flag：只调 `ensure_duckdb_table()` / SF `CREATE TABLE IF NOT EXISTS raw_dbt_run`，不要求 `--artifact`；同时把 SF 连接逻辑提取为 `_snowflake_connect()` + `ensure_snowflake_table()` 复用 helper。(b) `Makefile` demo / local-full / local-subset 三 target 在 `load_local.py` 之后、`dbt seed/run` 之前插入 `ingest_dbt_artifacts.py --init`。(c) `airflow/dags/nz_electricity_v2.py` 加第 5 个 dbt 阶段任务 `prepare_raw_dbt_run`（BashOperator，调 `--init --target snowflake`，trigger_rule=NONE_FAILED），放在 `check_run_dbt` 与 `run_dbt` 之间；`tests/test_dag_integrity.py` 把 v2 expected task set 从 16 task 扩到 17 task。**端到端**：DuckDB 干净部署 `make local-full` 等价流程实测：第一次 `dbt run` 27/27 PASS（无 ERROR / 无 SKIP），ingest 后 `fct_dbt_run` 单一 invocation 全为 success → **Model success rate = 100% ✅**；接着跑 `dbt test` + 再 ingest，Test pass rate = 140/141 = **99.29% ✅**（1 个 V1 既存 WARN 不算 pass）。**残留 caveat**：绕过 Makefile 直接跑 `dbt run` 的用户仍触发原始问题；workaround = 一次性手动跑 `python scripts/ingest_dbt_artifacts.py --init`。**PRD 更新**：§10.1 加 Known Limitation 条目（原状 + 修复 + caveat + 顺手记录 Pipeline Health 页 None 渲染为 ❌ 而非 ❓ 的 UX 小 bug，未修） |
...
>
> ✅ **6.0-impl 落地（2026-05-14）**：DS-3 Hydro 已实现并入栈。实际路径为 `Datasets/Environment/HydrologicalModellingDataset`（定期年度发布，最新 20241231），10 个 storage lake（NI×2 + SI×8）。详见 changelog `6.0-impl`。
...
> Q10（Hydro-Price Driver）随 DS-3 推迟到 V3。✅ **6.0-impl 落地**：`streamlit/pages/hydro_price_driver.py` 已上线（第 11 页，Hydro V3 分组）。Mart 用 `island × year_month` 粒度的 `mart_hydro_price_driver`，含 `storage_pct_of_max` window 派生指标 + 双 Y 轴 storage/price overlay + 每岛 Pearson r。
...
>
> ✅ **6.0-impl 落地**：Hydro 链路加回 4 个 model — `stg_hydro_storage` → `fct_hydro`（左联 `dim_catchment` seed-backed dim） → `mart_hydro_price_driver`（island × year_month grain，LEFT JOIN `mart_price_daily`）。DuckDB 当前实跑 **27 model**（V2 23 + V3 hydro 4；SF-only 的 `mart_warehouse_cost` 跳过）。
...
>
> ✅ **6.0-impl 落地**：第 4 条 ingest 分支 `hydro_download → hydro_upload → hydro_load` 已加入 V2 DAG（NONE_FAILED 触发，与 NSP 并列 best-effort），无 HMD release 时优雅跳过。topology 改为 `[g_load, p_load, n_load, h_load] >> t_check_dbt`。
...
- **Pipeline Health "model success" 启动期噪声**: 全新部署时 `stg_dbt_run` 依赖的 `raw_dbt_run` 表初始不存在，**首次 `dbt run` 必然让 stg_dbt_run/fct_dbt_run 失败一次**（1 error + 1 skipped）写入 `fct_dbt_run`，导致 Pipeline Health "Model success 30d" KPI 永久卡在 25/27 = **92.6%**（< 95% SLO ❌），需 ≥ 5 次干净 invocation 才能稀释回绿。**6.1-impl 修复**：`ingest_dbt_artifacts.py --init` 预创建空表；Makefile demo/local-full/local-subset 三 target + V2 DAG `prepare_raw_dbt_run` 任务自动调用。**残留 caveat**：绕过 Makefile 直接跑 `dbt run` 的用户仍会触发原始问题（手动跑 `python scripts/ingest_dbt_artifacts.py --init` 即可避开）。Pipeline Health 页 KPI #3 当 `test_pass_rate is None` 时显示 ❌ 而非 ❓（"无数据"/"低于 SLO"未区分）是一处独立 UX 小 bug，未修。
...
- ~~**Hydro Modelling Dataset + Q10 Hydro-Price Driver**~~ ✅ **Done in V3 (6.0-impl)** — `dim_catchment` seed + 4 dbt models（stg_hydro_storage / fct_hydro / mart_hydro_price_driver）+ 第 11 个 Streamlit 页（双 Y 轴 storage/price + Pearson r）。HMD 实际是 ≈ 年度 release（非月度批），路径 `Datasets/Environment/HydrologicalModellingDataset/`。NI×2 + SI×8 = 10 个 storage lake。实际工作量 < 1 天（V2 双模式骨架可直接套）。详见 changelog `6.0-impl`
...
| Hydro Modelling — landing page | `https://www.ea.govt.nz/data-and-insights/datasets/environment/hydrological-modelling-dataset/` |
| Hydro Modelling — Storage CSVs（V3 6.0-impl 实际使用） | `https://emidatasets.blob.core.windows.net/publicdata/Datasets/Environment/HydrologicalModellingDataset/3_StorageAndSpill_{YYYYMMDD}/3_1_Storage/{FILENAME}` |
| Hydro Modelling — Storage 文件索引 | `.../3_1_Storage/FileIndex_Storage.csv`（10 行：NI_TPO/NI_WKA + SI×8） |
